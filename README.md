...
## Архитектура: Monorepo и стратегия Multi-zones

Проект организован как монорепозиторий с прицелом на **Next.js Multi-zones**. Это осознанное решение, продиктованное необходимостью строить масштабируемую систему, где различные части продукта (например, лендинги и личный кабинет) могут развиваться независимо.

### Почему монорепозиторий и Multi-zones?

1.  **Независимость приложений**: Я считаю правильным выносить тяжелые части приложения (такие как `dashboard`) в отдельные Next.js инстансы. Это дает нативную возможность открывать их на разных поддоменах или путях (типа `/dashboard`), не раздувая основной бандл.
2.  **Эффективный CI/CD**: Благодаря монорепозиторию пересобирается и тестируется только то приложение, в котором были изменения. Это критично для скорости разработки в большой команде.
3.  **Единый источник истины**: Мы используем общую библиотеку UI-компонентов и утилит, что гарантирует визуальную консистентность. При этом приложения остаются изолированными — баг в админке не «уронит» страницу регистрации.
4.  **Возможности Next.js**: Использование этого фреймворка дает нам мощные инструменты прямо «из коробки»:
    - Проксирование запросов к бэкенду.
    - Сложные редиректы и проверки до того, как пользователь скачает JS-бандл.
    - Удобная работа с промо-страницами и SEO.

### Стек инфраструктуры:

- **Nx** — Выбран как основной тулчейн управления воркспейсом. В отличие от Turborepo, Nx предлагает более мощную систему генераторов и плагинов, что критично для поддержания консистентности кода.
- **pnpm** — Используется вместо npm/yarn для ускорения установки зависимостей и исключения `phantom dependencies` благодаря строгой структуре `node_modules`.
- **Next.js 15 (App Router)** — Выбрана версия 15, так как она уже устоялась и обладает стабильной поддержкой библиотек.

### Основные директории:

- `apps/main` — Основное приложение на Next.js (Auth flow).
- `libs/shared/ui` — Библиотека компонентов дизайн-системы.
- `.agents/` — Логи процесса разработки с использованием ИИ.

## Принятые технические решения

### 1. Стилизация: Tailwind CSS v4 + Semantic CSS Variables

Я выбрал новый Tailwind v4 за его CSS-first подход. Вместо хардкода значений в классах, я использую семантические переменные (`--color-brand`), что позволяет легко реализовать в будущем переключение тем (Light/Dark) и централизованно управлять дизайном.

### 2. Основа UI: Radix UI + CVA

- **Radix UI**: Позволяет не «изобретать велосипед» в плане доступности (A11y) и сосредоточиться на уникальном визуальном стиле Orbitto.
- **CVA (Class Variance Authority)**: Помогает декларативно управлять вариантами компонентов (error, sizes, states), избавляя от сложных условий в строках классов.

### 3. Качество кода

- **Husky & lint-staged**: Гарантируют, что в репозиторий попадает только код, прошедший проверки линтинга, форматирования и сообщениями коммитов соответствующими Conventional Commits.

### 4. Feature-Sliced Design (FSD) и границы слоев

Проект следует методологии **FSD**, которая навязывает строгую иерархию слоев. Мы не просто полагаемся на «честное слово» разработчиков, а принудительно ограничиваем импорты с помощью `eslint-plugin-boundaries`:

- **Строгая иерархия**: Слой `entities` никогда не сможет импортировать `features` или `pages`. Это предотвращает циклические зависимости и делает код предсказуемым.
- **Изоляция модулей**: Фичи и сущности изолированы друг от друга внутри своего слоя. Если фиче `auth` нужны данные из `user`, она получит их через композицию, а не через прямой импорт соседнего модуля.
- **Public API (index.ts)**: Все модули имеют строго определенные точки входа. Нельзя «залезть в кишки» модуля — это гарантирует, что внутренний рефакторинг фичи не сломает пол-приложения.

Подробнее о реализации границ и последних оптимизациях можно прочитать в логах агентов, например: [.agents/09-monorepo-optimization-and-multi-zones.md](.agents/09-monorepo-optimization-and-multi-zones.md).

## Backlog: Планы перед Production

1. **Visual Regression Testing (Loki / Chromatic)**: Автоматическое отслеживание визуальных регрессий.
2. **Observability (Sentry)**: Мониторинг ошибок и производительности в реальном времени.
