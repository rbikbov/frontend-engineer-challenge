...
Проект организован как монорепозиторий с использованием **Nx** и **pnpm**. Это обеспечивает эффективное управление зависимостями, кеширование задач и удобное разделение кода между приложениями и библиотеками.

### Основные директории:
- `apps/` — Содержит исполняемые приложения (например, основной фронтенд).
- `libs/` — Содержит переиспользуемые библиотеки (UI-kit, API-клиенты, утилиты).
- `package.json` — Корневой файл с основными зависимостями и скриптами Nx.
- `pnpm-workspace.yaml` — Конфигурация воркспейса для pnpm.
- `nx.json` — Конфигурация Nx.

### Как работать:
1. **Установка зависимостей**: `pnpm install`
2. **Запуск задач**: Используйте `npx nx <target> <project>` (например, `npx nx build web`).
3. **Граф зависимостей**: `npx nx graph` для визуализации связей в проекте.
